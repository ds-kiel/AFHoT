...
const u8_t INIT_CHM[5] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
...
/**
 * 	This method updates the channel map of a connection by using its local map and ANDing with the global channel map
 */
u8_t ll_conn_chm_combine(struct ll_conn *conn) {
	
	u8_t chm_tmp[5];
	u8_t *chm_conn = conn->conn_chan_map;
	u8_t *chm_global = ull_conn_chan_map_get();
	for( int i = 0; i <= 4; i++) {
		// Access to data_chan_map not thread safe?
		chm_tmp[i] = chm_conn[i] & chm_global[i];
	}
	memcpy(conn->llcp.chan_map.chm, chm_tmp,
...
/**
 *	This method sets the local channel map of a connection
 */
u8_t ll_conn_chm_update(u8_t *chm, struct ll_conn *conn) 
{
		...
		memcpy(conn->conn_chan_map, chm,
		       sizeof(conn->conn_chan_map));

		return ll_conn_chm_combine(conn);
}
		      ...
		      struct node_rx_ftr *ftr, struct lll_conn *lll)
{
	...
	// Init the channel map of the connection
  	memcpy(conn->conn_chan_map, INIT_CHM, sizeof(conn->conn_chan_map));
	...
	// struct k_timer chm_upd_timer;
	// printk("Vor Timer erstellen");
	if (afh_enabled) {
		struct timer_data *timer_data = get_timer_data(lll->handle);
		struct k_timer *timer = &timer_data->timer;
		timer_data->handle = lll->handle;
		k_timer_init(timer, handle_timer, handle_timer_stop);
		k_timer_user_data_set(timer, &lll->handle);
		k_timer_start(timer, CHM_UPDATE_INTERVAL, CHM_UPDATE_INTERVAL);
	}
	// Initialize local chan map
	for(int i = 0; i < 5; i++) {
		conn->conn_chan_map[i] = 0xff;
	}
