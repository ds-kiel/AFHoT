...
void lll_conn_isr_rx(void *param)
{
	...
	if (!trx_done) {
		radio_isr_set(isr_done, param);
		radio_disable();
		// WICHTIG Hier wurde ein Paket erwartet aber keins empfangen.
		return;
	}
...
lll_conn_isr_rx_exit:
	...
	if(!(crc_ok && pdu_data_rx->nesn != lll->sn)) {
		upd_conn_stat(lll->handle, lll_chan_get(), &(struct upd_data){.data_type = data_type_pdr, .data.pdr_action = ack});
	}
	...
	if (rssi_ready) {
		...
		// printk("Received something");
		// struct upd_data data;
		// data.data_type = data_type_rssi;
		// data.data.rssi = rssi;
		upd_conn_stat(lll->handle, lll_chan_get(), &(struct upd_data){.data_type = data_type_rssi, .data.rssi = rssi});
...
void lll_conn_isr_tx(void *param)
{
 ...
 upd_conn_stat(lll->handle, lll_chan_get(), &(struct upd_data){.data_type = data_type_pdr, .data.pdr_action = send});
}
