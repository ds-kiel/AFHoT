...
u8_t *ull_conn_chan_map_get(void)
{
	return data_chan_map;
}
...
static void conn_cleanup(struct ll_conn *conn, u8_t reason)
{
	...
	*((u8_t *)rx->pdu) = reason;
	...
	if (afh_enabled) {
		/* stop afh timer */
		k_timer_stop(&get_timer_data(lll->handle)->timer);
	}
			      ...
			      struct pdu_data *pdu)
{
  ...
  // char hex[10];
  // bin2hex(&pdu->llctrl.chan_map_ind.chm, 5, hex, 10);
  // printk("Channel map was set to: %d %d %d %d %d %d %d %d %d %d",hex[0],hex[1],hex[2],hex[3],hex[4],hex[5],hex[6],hex[7],hex[8],hex[9]);

  

  printk("Channel map was set to: %x %x %x %x %x\n", pdu->llctrl.chan_map_ind.chm[0], pdu->llctrl.chan_map_ind.chm[1],pdu->llctrl.chan_map_ind.chm[2], pdu->llctrl.chan_map_ind.chm[3],pdu->llctrl.chan_map_ind.chm[4] );
